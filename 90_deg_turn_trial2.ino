...
// if 1 or 2, wriggle back left; if -1 or -2, wriggle back right
int backLeft = 1;
int maxSameSideWriggles = 5;
...
void loop(){
  ...
  if (getAction() == 1) {
    ...
    if (backLeft >0 ) {
      ...
      delay(backTimeout);
      setMotors(1, defaultSpeed-50, 0, defaultSpeed+100);
      delay(backTime);

      if (backLeft == maxSameSideWriggles) {
        backLeft = -1;
      } else {
        backLeft++;
      }
      ...
      delay(backTimeout);
      setMotors(0, defaultSpeed+100, 1, defaultSpeed-50);
      delay(backTime);

      if (backLeft == -1*maxSameSideWriggles) {
        backLeft = 1;
      } else {
        backLeft--;
      }
